...
export default function ChatModal({ open, onOpenChange, booking }: ChatModalProps) {
  ...
  const [messages, setMessages] = useState<Array<{ from: "provider" | "customer"; text: string }>>([
    { from: "customer", text: "Hi, can I reschedule?" },
    { from: "provider", text: "Sure, what time works for you?" },
  ]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md bg-white/90 backdrop-blur-xl rounded-2xl border border-white/60">
        <DialogHeader>
          <DialogTitle>Chat with {booking.customer}</DialogTitle>
        </DialogHeader>
        <div className="h-64 overflow-y-auto space-y-2 p-2 bg-white/60 rounded-lg border">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.from === "provider" ? "justify-end" : "justify-start"}`}>
              <div className={`px-3 py-2 rounded-xl text-sm ${m.from === "provider" ? "bg-emerald-600 text-white" : "bg-gray-200 text-gray-800"}`}>
                {m.text}
              </div>
            </div>
          ))}
