...
export function HeroSection() {
  ...
  <div className="relative w-full max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 grid lg:grid-cols-2 gap-8 md:gap-12 items-start">
      ...
      <div className="grid grid-cols-3 gap-4 md:gap-6">
        {[
          ...
          <div key={i} className="bg-white/40 backdrop-blur-lg p-3 md:p-4 rounded-xl text-center shadow-sm">
            <state.icon className="h-4 w-4 md:h-6 md:w-6 text-emerald-600 mx-auto mb-1 md:mb-2" />
            <div className="text-lg md:text-xl font-bold">{state.number}</div>
            <p className="text-xs md:text-sm text-gray-600">{state.label}</p>
          </div>
        ))}
      </div>
    </div>
